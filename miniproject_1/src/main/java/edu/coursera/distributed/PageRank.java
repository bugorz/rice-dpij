    ...
    public static JavaPairRDD<Integer, Double> sparkPageRank(
        ...
        return sites
                ...
                .flatMapToPair(pair -> {
                    ...
                    double contribution = rank / website.getNEdges();

                    List<Tuple2<Integer, Double>> contributions = new ArrayList<>();

                    Iterator<Integer> destinationItr = website.edgeIterator();
                    while (destinationItr.hasNext()) {
                        Integer destination = destinationItr.next();
                        contributions.add(new Tuple2<>(destination, contribution));
                    }
                    return contributions;
                })
                .reduceByKey((v1, v2) -> v1 + v2)
                .mapValues(v -> 0.85 * v + 0.15);
    }
}
